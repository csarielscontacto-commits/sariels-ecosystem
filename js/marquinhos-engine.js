...
export const Engine = {
    ...
    obtenerAnunciosActivos: async () => {
        try {
            ...
            const { data } = await window.supabase.from('publicidad_marquinhos').select('*').eq('pago_confirmado', true).lte('fecha_inicio', ahora).gte('fecha_fin', ahora);
            return data || [];
        } catch { return []; }
    },
    ...
    registrarVisualizacion: async (anuncioId) => {
        try {
            const { data: actual } = await window.supabase.from('publicidad_marquinhos').select('veces_mostrado').eq('id', anuncioId).single();
            if(!actual) return;
            await window.supabase.from('publicidad_marquinhos').update({ veces_mostrado: (actual.veces_mostrado || 0) + 1 }).eq('id', anuncioId);
        } catch {}
    },
    registrarClick: async (anuncioId) => {
        try {
            const { data: actual } = await window.supabase.from('publicidad_marquinhos').select('veces_click').eq('id', anuncioId).single();
            if(!actual) return;
            await window.supabase.from('publicidad_marquinhos').update({ veces_click: (actual.veces_click || 0) + 1 }).eq('id', anuncioId);
        } catch {}
    },
    comprarPublicidad: async ({ empresa, mensaje, url, nivel, duracionDias }) => {
        ...
        const { data, error } = await window.supabase.from('publicidad_marquinhos').insert({
